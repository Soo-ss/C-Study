/*
문자열자체는 const 상수로 인식 합니다. 상수는 변수가 아니기 때문에 값을 대입하거나 변경 할수 없습니다.
그래서 상수문자열 자체의 주소를 넘겨줄때 오류가 생기는 것입니다.
const char* pText = "테스트 문자열";  이렇게 구문을 작성하시면 되겠습니다.
이런 부분들은 컴파일 버전마다 다른 부분이 있습니다. 2017은 정확히 const 변수에 대해 오류처리를 해주네요﻿
*/
...
enum JOB
{
	...
	JOB_ARCHER,
	JOB_WIZARD,
	JOB_END		// JOB_END를 추가한 이유는 직업이 추가돼도 관리가 편함.
};
...
struct _tagItem
{
	...
	int iPrice;
	int iSell;
	char strDesc[ITEM_DESC_LENGTH]; // Desc = Descripton
};
...
void SetMonster(_tagMonster* pMonsterArr)
{
	...
	pMonsterArr[2] = CreateMonster("드래곤", 250, 500, 200, 400, 30000, 20000, 10, 30000, 20000, 1500);
}
...
void OutputMonster(_tagMonster* pMonster)
{
	...
	cout << "공격력 : " << pMonster->iAttackMin << " ~ " << pMonster->iAttackMax << "\t방어력 : " << pMonster->iArmorMin <<
		" ~ " << pMonster->iArmorMax << endl;
	cout << "체력 : " << pMonster->iHP << " / " << pMonster->iHPMax << "\t마나 : " << pMonster->iMP <<
		" / " << pMonster->iMPMax << endl;
...
void Battle(_tagPlayer* pPlayer, _tagMonster* pMonster)
{
	...
	iDamage = iDamage < 1 ? 1 : iDamage; // 최소 데미지를 1로 줌.
